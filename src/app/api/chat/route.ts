...
import { decodeOpenRouterStream, openChatStream } from "@/lib/server/openrouter";
...
import { createSseChatStream, localChatText, SSE_HEADERS } from "@/lib/server/sse";
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    const providerStream = runtime.openRouterKey
      ? decodeOpenRouterStream(
          await openChatStream(
            runtime.openRouterKey,
            buildRagMessages(input.query, sources),
            { maxTokens: 1_200 },
          ),
        )
      : null;
    const stream = createSseChatStream(
      providerStream,
      sources,
      providerStream ? undefined : localChatText(input.query, sources),
    );
