...
## Atlas counsel behaviour

The system prompt is fixed server-side in `src/lib/server/prompts.ts` (not exposed in the lawyer-facing UI). Behaviour:

- Non-sycophantic, facts-first tone
- Statute-first answers when written law applies
- Paper-analysis framework (what it is / applicability / use / precedents)
- No invented authorities
- Inline `[[docId|label]]` wikilinks so mid-answer cites open the paper (footer chips too)
- Client also linkifies bare neutral citations such as `[2007] SGCA 37`
...
5. Open Atlas counsel and ask a duty-of-care or s 112 question; click an inline cite to open the paper.
